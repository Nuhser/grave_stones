...
# summon grave marker
execute as @a[scores={gs_deaths=1..}] at @s if entity @e[type=item,distance=..5,scores={gs_itemAge=0..1}] align y run function grave_stones:_find_pos

# save the dropped items
execute as @a[scores={gs_deaths=1..}] at @s if entity @e[type=item,distance=..5,scores={gs_itemAge=0..1}] run function grave_stones:_save_items
